...
Simple::Simple(void *base, size_t size)
    : _base(base), _base_len(size), _data_end((char *)base) {
...
void Simple::print_sys() {
  ...
  for (int i = 0; i < sys_len; i += 2) {
    // cout << "SYS: " << this->_sys_start[-i] << ' ' << this->_sys_start[-i -
    // 1] << endl;
  }
  // cout << "\n\n";
...
void Simple::print_queue() {
  ...
  while (queue != nullptr) {
    // cout << "QUEUE: q, *q, *(q+1) = " << queue << ' ' << *((void **)queue) <<
    // ' ' << *((size_t *) queue + 1) << endl;
    queue = *((void **)queue);
  }
}
...
Pointer Simple::alloc(size_t N) {
  ...
  if (!flag) {
    ...
    // cout << "!flag and idx = " << ((char *) queue) - ((char *) this->_base)
    // << endl;
...
void Simple::defrag() {
  ...
  for (;;) {
    ...
    memmove(defraged_end, (char *)this->_base + mmin,
            this->_sys_start[-idx - 1]);
    defraged_end += this->_sys_start[-idx - 1];
  ...
  if (defraged_end < (char *)(this->_sys_end - 2)) {
    ...
    *((size_t *)this->_top + 1) =
        this->_base_len - (defraged_end - (char *)this->_base);
  }
